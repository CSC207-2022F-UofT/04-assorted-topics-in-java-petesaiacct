...
class DrivableMap {
    ...
    public List<Tradable> getTradable(){
        List<Tradable> objects = new ArrayList<>();
        for (String x : this.drivable_map.keySet()){
            if (this.drivable_map.get(x) instanceof Tradable){
                objects.add((Tradable) this.drivable_map.get(x));
            }
        } return objects;
    }
